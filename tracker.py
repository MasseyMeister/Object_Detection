...
class Tracker:
    """
    Greedy Tracker with tracking based on ``centroid``
    location of the bounding box of the object.
    This tracker is also referred as ``CentroidTracker`` in this repository.

    Args:
        max_lost (int): Maximum number of
        consecutive frames object was not detected.
    ...
    def _add_track(self, frame_id, bbox, detection_confidence,
                   class_id, **kwargs):
        """
        ...
        Args:
            ...
            bbox (numpy.ndarray): Bounding box pixel coordinates
            as (xmin, ymin, xmax, ymax) of the track.
            detection_confidence (float): Detection
            confidence of the object (probability).
            class_id (str or int): Class label id.
            kwargs (dict): Additional key word arguments.
        """

        self.tracks[self.next_track_id] = Track(
            self.next_track_id, frame_id, bbox,
            detection_confidence, class_id=class_id,
    ...
    def _update_track(self, track_id, frame_id, bbox, detection_confidence,
        ...
        Args:
            ...
            bbox (numpy.ndarray or list): Bounding box
            coordinates as `(xmin, ymin, width, height)`.
            detection_confidence (float): Detection
            confidence (a.k.a. detection probability).
            class_id (int): ID of the class (aka label)
        ...
        self.tracks[track_id].update(
            frame_id, bbox, detection_confidence, class_id=class_id,
    ...
    def _get_tracks(tracks):
        ...
        Args:
            tracks (OrderedDict): Tracks dictionary with (key, value)
    ...
    def preprocess_input(bboxes, class_ids, detection_scores):
        ...
        Args:
            bboxes (list or numpy.ndarray): Array of bounding boxes
            with each bbox as a tuple containing `(xmin, ymin, width, height)`.
        ...
        Returns:
            detections (list[Tuple]): Data for detections as list
            of tuples containing `(bbox, class_id, detection_score)`.
        """
        ...
        new_detections = list(zip(new_bboxes, new_class_ids,
    ...
    def update(self, bboxes, detection_scores, class_ids):
        ...
        Args:
            ...
            detection_scores(numpy.ndarray or list): List of
            detection scores (probability) of each detected object.
        ...
        detections = Tracker.preprocess_input(bboxes, class_ids,
                                              detection_scores)
        ...
        if len(track_ids):
            ...
            centroid_distances = spatial.distance.cdist(
                track_centroids,
                detection_centroids)
